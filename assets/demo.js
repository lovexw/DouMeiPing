...
(function () {
  ...
  var STORES = {
    ...
    tianmao: {
      id: 'tianmao', code: '789', icon: '🚗',
      brand: '天猫养车（银河南街店）',
      name: '天猫养车 · 演示门店',
      desc: '天猫养车 · 门店运营工具',
      industry: 'carwash',
      wifi: { ssid: 'TMYC-5G', password: 'tmyc88888888' },
    }
  };

  var INDUSTRY_LABELS = {
    coffee: '咖啡饮品', hotpot: '火锅店', chaocai: '炒菜馆', bbq: '烧烤店',
    massage: '足疗按摩', fusion: '融合菜', roastduck: '烤鸭店', carwash: '汽车养护', generic: '通用行业'
  };

  /* ---------- 分行业词库 ----------
   * 每个行业两套风格：normal 真实评价风 / xhs 小红书探店风；
   * carwash 额外有 plain 简洁实在风（短句、只写事实，不带装饰性修饰）。
  ...
  var LIBS = {
    ...
    carwash: {
      normal: {
        openers: [
          '周末过来洗了个车，从预冲到交车全程看着做完',
          '同事推荐这家天猫养车，第一次来洗车',
          '车顶落了层树胶鸟粪，来银河南街这家店处理',
          '家附近新找的养车门店，过来试了次精洗',
          '新车上牌后第一次洗车，选的这家',
          '办了次卡，这次带家里第二台车来',
          '下过雨车身全是泥点，路过就进来洗了',
        ...
        cores: [
          '高压水枪先冲一遍，泥沙冲干净了才动手擦，不划漆',
          '轮毂缝隙、车标、门框胶条都用小刷子刷到',
          '内饰吸尘后，中控、杯架、出风口挨个擦到位',
          '泡沫覆盖得均匀，冲完车身没有水痕',
          '脚垫单独拿出去高压冲洗，晾干再装回去',
          '前挡和后视镜擦得透亮，雨刮片也顺手擦了',
          '车漆上的柏油点用药水点掉了，摸上去不剌手',
          '门缝和后备箱封条里的灰也清了出来'
        ],
        experiences: [
          '到店不用排队，二十来分钟洗完交车',
  ...
  function generateReview(industry, style) {
    var libs = LIBS[industry] || LIBS.coffee;
    var lib = libs[style] || libs.normal;
  ...
  function bootDemo() {
    ...
    var styleLabelMap = { normal: '真实评价风', xhs: '小红书探店风', plain: '简洁实在风' };
